...
#install.packages("spData")       ## uncomment to install spData for World map (and other maps)
```
...
library(spData)                 ## provides World map
```
...
player_cities <- st_as_sf(player_cities, coords = c("lng", "lat"), crs = 3857)
```
...
m1 <- tm_shape(world)+
  ...
  tm_dots("false_count",
          ## set size to count column values
          size = "count",
...
```{r save maps, echo = FALSE, message = FALSE, warning = FALS}
...
tmap_save(tm = m1, filename = here("/graphics/map1.eps"), width = 2560, height = 1440, asp = 0)
```
